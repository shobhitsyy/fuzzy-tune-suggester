...
/**
 * Song Detail modal with dynamic backend-powered similar songs.
 */
const SongDetail: React.FC<SongDetailProps> = ({
  ...
  useEffect(() => {
    ...
    setLoading(true);
    getSimilarSongs(song.id, 3).then((res) => {
      setSimilarSongs(res);
      setLoading(false);
    });
  }, [song, isOpen]);
      ...
      <DialogContent className="max-w-3xl p-0 gap-0 rounded-xl overflow-hidden border-0 shadow-xl">
        ...
        <ScrollArea className="p-6 max-h-[60vh]">
          <div className="grid md:grid-cols-2 gap-8">
            ...
            <div className="space-y-4">
              <DialogDescription className="text-foreground min-h-[40px]">
                {song.description}
              </DialogDescription>

              {loading ? (
                <div className="text-sm text-gray-400 mt-8">Loading similar songs...</div>
